...
export class SequenceNumberCache {
  ...
  set(accountId: string, sequenceNumber: string): void {
    ...
    console.debug(`[SequenceCache] Cached sequence ${sequenceNumber} for account ${accountId}`);
  }
  ...
  increment(accountId: string): void {
    const entry = this.cache.get(accountId);
    if (!entry) {
      console.warn(`[SequenceCache] Cannot increment sequence for uncached account ${accountId}`);
      return;
    }
    ...
    console.debug(`[SequenceCache] Incremented sequence to ${newSequence} for account ${accountId}`);
  }
  ...
  invalidate(accountId: string): void {
    this.cache.delete(accountId);
    console.debug(`[SequenceCache] Invalidated sequence cache for account ${accountId}`);
  }
  ...
  private releaseLock(accountId: string): void {
    ...
    console.debug(`[SequenceCache] Released lock for account ${accountId}, queue length: ${entry.lockQueue.length}`);
  }
  ...
  clear(): void {
    this.cache.clear();
    console.debug('[SequenceCache] Cleared all sequence number cache');
  }
  ...
  async executeWithSequenceManagement<T>(
    ...
    try {
      ...
      if (cachedSequence) {
        ...
        console.debug(`[SequenceCache] Using cached sequence ${cachedSequence} for ${accountId}`);
      } else {
        // Fetch from network
        account = await fetchAccount();
        const networkSequence = account.sequenceNumber();
        this.set(accountId, networkSequence);
        console.debug(`[SequenceCache] Fetched fresh sequence ${networkSequence} for ${accountId}`);
      }
      ...
      try {
        ...
        if (this.isSequenceMismatchError(error)) {
          console.warn(`[SequenceCache] Sequence mismatch for ${accountId}, refreshing and retrying`);
          ...
          console.debug(`[SequenceCache] Retrying with fresh sequence ${freshSequence}`);
  ...
  private isSequenceMismatchError(error: any): boolean {
    ...
    if (message.includes('tx_bad_seq')) return true;
    ...
    // Check extras from Horizon errors
    if (error.response?.data?.extras) {
      const extras = JSON.stringify(error.response.data.extras).toLowerCase();
      if (extras.includes('tx_bad_seq')) return true;
      if (extras.includes('bad_seq')) return true;
    }
