...
describe('SequenceNumberCache', () => {
  ...
  describe('Sequence mismatch detection', () => {
    ...
    it('should detect tx_bad_seq in message', () => {
      const error = { message: 'Transaction error: tx_bad_seq' };
      expect((cache as any).isSequenceMismatchError(error)).toBe(true);
    });
    ...
    it('should detect tx_bad_seq in response extras', () => {
      const error = {
        message: 'Transaction failed',
        response: {
          data: {
  ...
  describe('executeWithSequenceManagement', () => {
    ...
    it('should handle mixed success and failure scenarios', async () => {
      ...
      const buildAndSubmit = jest
        ...
        .mockRejectedValueOnce({ message: 'tx_bad_seq' })
        .mockResolvedValueOnce({ hash: 'tx2-retry' });

      // First transaction succeeds
      const result1 = await cache.executeWithSequenceManagement(
        accountId,
        fetchAccount,
        buildAndSubmit
      );
      expect(result1).toEqual({ hash: 'tx1' });
      expect(cache.get(accountId)).toBe('101');

      // Second transaction fails with bad_seq and retries
      const result2 = await cache.executeWithSequenceManagement(
        accountId,
        fetchAccount,
        buildAndSubmit
      );
      expect(result2).toEqual({ hash: 'tx2-retry' });
      expect(cache.get(accountId)).toBe('106'); // Fresh sequence + 1
    });
  });
